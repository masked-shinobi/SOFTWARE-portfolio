...
// =============================================================================
// ChoiceBackgroundCanvas — Three.js Spotlight Background Reveal
// =============================================================================
// - Renders the blueprint background (dark_background.png / light_background.png).
// - The background remains fully hidden by default.
// - An interactive radial spotlight around the mouse cursor reveals the blueprint.
// - Smooth linear interpolation (lerp) ensures fluid 60fps cursor-following movement.
// - Soft feathered falloff edge for a premium look.
// =============================================================================

interface ChoiceBackgroundCanvasProps {
  theme?: "dark" | "light";
  radius?: number;
  falloff?: number;
  className?: string;
...
void main() {
  // Keep native 1:1 scale centered on screen and stack/tile across width and height (never expand)
  ...
  // Calculate distance from cursor in screen pixel space
  float dist = distance(gl_FragCoord.xy, u_mouse);

  // Spotlight radial mask with soft falloff
  ...
  // Blend background color into blueprint texture
  vec3 finalColor = mix(u_bgColor, texColor.rgb, mask);
  gl_FragColor = vec4(finalColor, 1.0);
}
`;

export function ChoiceBackgroundCanvas({
  theme: propTheme,
  radius = 280,
  falloff = 0.55,
  className = "",
}: ChoiceBackgroundCanvasProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const [detectedTheme, setDetectedTheme] = useState<"dark" | "light">("dark");
  const activeTheme = propTheme || detectedTheme;

  // Detect system theme preference if not passed explicitly
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const bgVec = isDark
      ? new THREE.Vector3(0.0, 0.0, 0.0) // Pure black for dark mode
      : new THREE.Vector3(1.0, 1.0, 1.0); // Pure white for light mode
    ...
    const uniforms = {
      ...
      u_intensity: { value: 0.0 }, // Starts hidden, fades in on hover/interaction
      u_texSize: {
    ...
    const pointerToLocal = (e: PointerEvent) => {
      const rect = container.getBoundingClientRect();
      const width = rect.width || 1;
      const height = rect.height || 1;
      return {
        x: ((e.clientX - rect.left) * container.clientWidth) / width,
        y: ((e.clientY - rect.top) * container.clientHeight) / height,
      };
    };

    const handlePointerMove = (e: PointerEvent) => {
      const local = pointerToLocal(e);
      targetMouseX = local.x;
      targetMouseY = local.y;
      targetIntensity = 1.0;
    };
    ...
    const handlePointerLeave = () => {
      targetIntensity = 0.0; // Smoothly fades out when leaving window
    };
